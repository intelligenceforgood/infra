# ---------------------------------------------------------------------------
# PII-Vault Stack — Vault Cloud Run Service
#
# Deploys the vault API as a Cloud Run service. Only enabled in environments
# that need the vault API (e.g. dev). Gated behind var.deploy_vault_service.
# ---------------------------------------------------------------------------

resource "google_service_account" "vault_sa" {
  count        = var.deploy_vault_service ? 1 : 0
  project      = var.project_id
  account_id   = "sa-vault"
  display_name = "Vault Service Account"
}

# Grant Cloud SQL Client
resource "google_project_iam_member" "vault_sql_client" {
  count   = var.deploy_vault_service ? 1 : 0
  project = var.project_id
  role    = "roles/cloudsql.client"
  member  = "serviceAccount:${google_service_account.vault_sa[0].email}"
}

# Grant Secret Access
resource "google_secret_manager_secret_iam_member" "vault_secret_access" {
  count     = var.deploy_vault_service ? 1 : 0
  project   = var.project_id
  secret_id = module.tokenization_secrets.secret_ids["pii_key"]
  role      = "roles/secretmanager.secretAccessor"
  member    = "serviceAccount:${google_service_account.vault_sa[0].email}"
}

resource "google_secret_manager_secret_iam_member" "vault_pepper_access" {
  count     = var.deploy_vault_service ? 1 : 0
  project   = var.project_id
  secret_id = module.tokenization_secrets.secret_ids["pepper"]
  role      = "roles/secretmanager.secretAccessor"
  member    = "serviceAccount:${google_service_account.vault_sa[0].email}"
}

# Grant KMS Access
resource "google_kms_crypto_key_iam_member" "vault_kms_access" {
  count         = var.deploy_vault_service ? 1 : 0
  crypto_key_id = google_kms_crypto_key.vault_key.id
  role          = "roles/cloudkms.cryptoKeyEncrypterDecrypter"
  member        = "serviceAccount:${google_service_account.vault_sa[0].email}"
}

# Vault SA database user
resource "google_sql_user" "vault_sa_user" {
  count    = var.deploy_vault_service ? 1 : 0
  name     = trimsuffix(google_service_account.vault_sa[0].email, ".gserviceaccount.com")
  instance = google_sql_database_instance.vault.name
  project  = var.project_id
  type     = "CLOUD_IAM_SERVICE_ACCOUNT"
}

# Grant the Vault Service Account access to login (Cloud SQL Instance User)
resource "google_project_iam_member" "vault_sa_sql_instance_user" {
  count   = var.deploy_vault_service ? 1 : 0
  project = var.project_id
  role    = "roles/cloudsql.instanceUser"
  member  = "serviceAccount:${google_service_account.vault_sa[0].email}"
}

module "vault_service" {
  count  = var.deploy_vault_service ? 1 : 0
  source = "../../modules/run/service"

  depends_on = [google_project_service.run]

  project_id      = var.project_id
  name            = "i4g-vault"
  location        = var.region
  service_account = google_service_account.vault_sa[0].email
  image           = var.vault_service_image

  env_vars = {
    "I4G_ENV"                            = var.environment
    "I4G_STORAGE__STRUCTURED_BACKEND"    = "cloudsql"
    "I4G_APP__CLOUDSQL__INSTANCE"        = google_sql_database_instance.vault.connection_name
    "I4G_APP__CLOUDSQL__DATABASE"        = google_sql_database.vault_db.name
    "I4G_APP__CLOUDSQL__USER"            = trimsuffix(google_service_account.vault_sa[0].email, ".gserviceaccount.com")
    "I4G_APP__CLOUDSQL__ENABLE_IAM_AUTH" = "true"
    "I4G_PII__BACKEND"                   = "cloudsql"
    "I4G_PII__CLOUDSQL__INSTANCE"        = google_sql_database_instance.vault.connection_name
    "I4G_PII__CLOUDSQL__DATABASE"        = google_sql_database.vault_db.name
    "I4G_PII__CLOUDSQL__USER"            = trimsuffix(google_service_account.vault_sa[0].email, ".gserviceaccount.com")
    "I4G_PII__CLOUDSQL__ENABLE_IAM_AUTH" = "true"
  }

  secret_env_vars = {
    "I4G_PII__PEPPER" = {
      secret  = module.tokenization_secrets.secret_names["pepper"]
      version = "latest"
    }
    "I4G_CRYPTO__PII_KEY" = {
      secret  = module.tokenization_secrets.secret_names["pii_key"]
      version = "latest"
    }
  }
}
