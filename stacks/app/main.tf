...
# App Stack — Main
#
# Unified Terraform for the app environment (dev + prod). Environment-specific
# behaviour is driven by var.environment and conditional flags derived in
# locals.tf.  Values (project IDs, images, domains, etc.) come from the
# thin environment wrappers via terraform.tfvars.
# ---------------------------------------------------------------------------
...
module "run_core_svc" {
  ...
  env_vars = merge(
    ...
    # IAP backend audience — only available when LB is deployed.
    local.deploy_lb ? {
      I4G_IDENTITY__IAP_BACKEND_AUDIENCE = format("/projects/%s/global/backendServices/%s", data.google_project.current.number, module.global_lb[0].backend_service_ids["api"])
    } : {},
...
module "run_console" {
  ...
  env_vars = merge(
    ...
    # SSI_API_URL — required for the eCX proxy routes (feed, search,
    # submissions, polling-status, stats) which forward directly to ssi-svc.
    # The investigate/poll routes still proxy through core's API, but eCX
    # routes have no core-side fallback.
    var.ssi_service_enabled ? {
...
# SSI runs as a Cloud Run Service; core triggers investigations via HTTP POST.
# Gated behind var.ssi_service_enabled (default: true).
  ...
  backends = merge(
    {
      api = {
        ...
        service_name      = "core-svc" # literal to break cycle with run_core_svc
        region            = var.region
