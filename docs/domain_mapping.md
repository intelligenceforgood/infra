...
- Custom domains:
  - Dev: `api.dev.intelligenceforgood.org` -> Core API, `app.dev.intelligenceforgood.org` -> `i4g-console`
  - Prod: `api.intelligenceforgood.org` -> Core API, `app.intelligenceforgood.org` -> `i4g-console`
- Terraform will create Cloud Run domain mappings when `core_svc_custom_domain` or `ui_custom_domain` variables are set.
...
- Create CNAME records for each environment:
  - Dev: `api.dev.intelligenceforgood.org` -> `ghs.googlehosted.com`
  - Dev: `app.dev.intelligenceforgood.org` -> `ghs.googlehosted.com`
  - Prod: `api.intelligenceforgood.org` -> `ghs.googlehosted.com`
  - Prod: `app.intelligenceforgood.org` -> `ghs.googlehosted.com`
- Wait for DNS propagation, then run terraform apply to create the domain mapping.

## IAP & OAuth

- If using IAP, create an OAuth brand + OAuth client manually via Google Cloud Console.
- Add authorized redirect URIs and allowed origins for the custom domains (e.g. `https://api.dev.intelligenceforgood.org/_gcpgtoken` for dev, `https://api.intelligenceforgood.org/_gcpgtoken` for prod, or application-specific callback endpoints).
...
- Once mapped and DNS updated, confirm:
  - Visit `https://api.dev.intelligenceforgood.org` (dev) or `https://api.intelligenceforgood.org` (prod) — should present IAP authorization if enabled.
  - Run `gcloud run domain-mappings describe api.dev.intelligenceforgood.org --region us-central1 --project i4g-dev` to verify the dev mapping.
